...
<c:import url="/WEB-INF/views/layout/app.jsp">
    <c:param name="content">
                ...
                <c:forEach var="report" items="${reports}" varStatus="status">
                    ...
                    <tr class="row${status.count % 2}">
                        ...
                        <td class="report_action">
                            <a href="<c:url value='?action=${actRep}&command=${commShowApp}&id=${report.id}'/>">詳細を見る</a>
                        </td>
                        <td class="report_approval">
                            <c:choose>
                            <c:when test="${report.approvalFlag==AttributeConst.REMAND.getIntegerValue()}">差し戻し</c:when>
                            <c:otherwise>
                                <c:choose>
                                <c:when test="${report.approvalFlag==AttributeConst.APPROVED.getIntegerValue()}">承認済み</c:when>
        ...
        <div id="pagination">（全 ${reports_count} 件）<br/>
